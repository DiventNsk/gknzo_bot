...
async function takeScreenshots() {
  ...
  try {
    ...
    const reportsButton = await page.locator('.fixed .touch-manipulation:has-text("Отчеты")').first();
    if (await reportsButton.count() > 0) {
      await reportsButton.click();
    } else {
      console.log('Кнопка "Отчеты" не найдена в нижней навигации');
    }
    ...
    const fillButton = await page.locator('.fixed .touch-manipulation:has-text("Заполнить")').first();
    if (await fillButton.count() > 0) {
      await fillButton.click();
    } else {
      console.log('Кнопка "Заполнить" не найдена в нижней навигации');
    }
    ...
    const directorButton = await page.locator('.fixed .touch-manipulation:has-text("Директор")').first();
    if (await directorButton.count() > 0) {
      await directorButton.click();
    } else {
      console.log('Кнопка "Директор" не найдена в нижней навигации');
    }
